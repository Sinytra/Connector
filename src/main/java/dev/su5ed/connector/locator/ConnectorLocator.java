...
public class ConnectorLocator extends AbstractJarFileModProvider implements IModLocator {
    ...
    public List<IModLocator.ModFileOrException> scanMods() {
        ...
        Stream<IModLocator.ModFileOrException> fabricJars = moduleSafeJars.stream()
            .map(info -> new ModFileOrException(createConnectorModFile(info, this), null));
    ...
    public static IModFile createConnectorModFile(SplitPackageMerger.ModPath info, IModProvider provider) {
        ...
        IModFile mod = new ModFile(modJar, provider, modFile -> ConnectorModMetadataParser.fabricModJsonParser(modFile, info.metadata()));
        mjm.setModFile(mod);
        return mod;
    }
     ...
     */
    record FabricModFileMetadata(LoaderModMetadata modMetadata, Map<String, String> mixinConfigs, Set<String> refmaps, Set<String> mixinClasses, Attributes manifestAttributes) {}

    record FabricModPath(Path path, FabricModFileMetadata metadata) {}
}
